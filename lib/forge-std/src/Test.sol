...
abstract contract Test is DSTest, Script {
    ...
    function tip(address token, address to, uint256 give) public {
        emit WARNING_Deprecated(
            "The `tip` stdcheat has been deprecated. Use `deal` instead."
        );
    ...
    function find(StdStorage storage self) internal returns (uint256) {
        ...
        if (reads.length == 1) {
            ...
            if (fdat != curr) {
                require(
                    false,
                    "stdStorage find(StdStorage): Packed slot. This would cause dangerous overwriting and currently isnt supported"
                );
            }
            emit SlotFound(
                who,
